...
Public Class PTDetailGridWrapper
    ...
    Public Event AfterLoadGridDetails(ByVal GridName As String)
    ...
    Public Sub New(ByVal FormDetailGrid As DetailGrid)
        Try
            ...
            Try
                ...
                With DirectCast(Me.DetailGrid.FindForm, CRUDBase)
            ...
            CreateDetailGridColumns(Me.DetailGrid, DetailColumns)
            ...
            Try
                ...
                If Me.DetailGrid.ShowDeleteButton Then
                    ...
                    AddHandler repDelete.ButtonPressed, AddressOf Delete_ButtonPressed
                    Dim grdColumn = DetailGridView.Columns.AddField("")
                    With grdColumn
                        .Caption = ""
                        .Width = 20
    ...
    Private Sub GridLookupEdit_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            ActiveLookUpEditColumns = GetColumnInfo(CType(CType(Me.DetailGrid.FindForm, CRUDBase).GetControl(CType(edit.Tag, RepositoryItemGridLookUpEdit).Tag), GridPickListInfo)._PickColumnsInfo)
    ...
    Private Sub CreatePickLists()
        Try
            Dim ownerForm As CRUDBase = DirectCast(Me.DetailGrid.FindForm, CRUDBase)
                    ...
                    Using cmd As DbCommand = conn.CreateCommand
                        ...
                        With DetailGridData.Tables("TableData")
                            For Each DeletedRow As DataRow In DeletedRows
                                ...
                                Try
                                    If InStr(DetailPKField.ToLower, "ldgrentries") AndAlso DetailGrid.GLEnabled Then
                                        UpdateCOABalance(DeletedRow("fk_coa_ldgrentries", DataRowVersion.Original))
                                    End If
                                Catch ex As Exception
                    ...
                    Using cmd As DbCommand = conn.CreateCommand
                        ...
                        With DetailGridData.Tables("TableData")
                            ...
                            For Each ModifiedRow As DataRow In ModifiedRows
                                ...
                                Try
                                    If InStr(DetailPKField.ToLower, "ldgrentries") AndAlso DetailGrid.GLEnabled Then
                                        UpdateCOABalance(ModifiedRow("fk_coa_ldgrentries", DataRowVersion.Current))
                                    End If
                                Catch ex As Exception
                        ...
                        With DetailGridData.Tables("TableData")
                            For Each AddedRow As DataRow In AddedRows
                                ...
                                Try
                                    ...
                                    Try
                                        If InStr(DetailPKField.ToLower, "ldgrentries") AndAlso DetailGrid.GLEnabled Then
                                            UpdateCOABalance(AddedRow("fk_coa_ldgrentries", DataRowVersion.Current))
                                        End If
                                    Catch ex As Exception
        ...
        Try
            ...
            If Not Me.DetailColumns Is Nothing Then
                ...
                For Each FieldName As Object In FieldNames
                    ...
                    Dim FieldSuffix As String = Field.Split("_")(UBound(Field.Split("_"))).ToLower

                    DisplaySelectString &= Field + ","

                    If TableSuffix = FieldSuffix Then
    ...
    Private Sub DataReadyCallBack(ByVal ar As IAsyncResult)
        Try
            ...
            Else
                ...
                End With
            End If
            RaiseEvent AfterLoadGridDetails(Me.DetailGrid.Name)
        Catch ex As Exception
        ...
        Try
            ...
            If Not IsNothing(Me.DetailColumns) Then
                ...
                For Each col As ColumnInfo In Me.DetailColumns
                    ...
                    If col.Caption.ToLower = "pk" Then
                        pkField = col.FieldName
                        Me.DetailGridView.SetRowCellValue(e.RowHandle, Me.DetailGridView.Columns(col.FieldName), GetPK)
                    ElseIf col.Caption.ToLower = "fk" Then
                    ...
                    Select Case defval
                        ...
                        Case Else
                            ...
                            If TypeOf (Me.DetailGrid.FindForm) Is CRUDBase AndAlso defval <> "" Then
                                tmpControl = CType(Me.DetailGrid.FindForm, CRUDBase).GetControl(defval)
                            End If
                            ...
                            Else
                                ...
                                Or defval.Contains("add") Or defval.Contains("sub") _
                                    Or defval.Contains("div") Or defval.Contains("pk") Or defval.Contains("fk") Or defval.Contains("storepk") Or defval = String.Empty) Then
            ...
            Select Case True
                ...
                Case e.Control AndAlso e.KeyCode = Keys.Delete
                    ...
                    Try
                        ...
                        Dim ctrl = DirectCast(Me.DetailGrid.FindForm, CRUDBase).GetControl(TotalField)
                        Select Case TypeName(ctrl)
                            Case "PTGridLookupEdit"
                                DirectCast(ctrl, PTGridLookUpEdit).EditValue = tmpColumn.SummaryText.Split(" ")(1)
                            Case "PTTextInput", "PTUserPKInput"
                                DirectCast(ctrl, PTTextInput).Text = tmpColumn.SummaryText.Split(" ")(1)
                            Case "PTMemoInput"
                                DirectCast(ctrl, PTMemoInput).EditValue = tmpColumn.SummaryText.Split(" ")(1)
                            Case "PTComboBoxInput"
                                DirectCast(ctrl, PTComboBoxInput).Text = tmpColumn.SummaryText.Split(" ")(1)
                        End Select
    ...
    Private Sub DetailGrid_ShownEditor(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            With DirectCast(sender, GridView)
                Try
                    If TypeOf (.ActiveEditor) Is PTGridLookUpEdit Then
        ...
        If Me.DetailGridView.OptionsBehavior.Editable = True Then
            Try
                ...
                If e.RowHandle <> DevExpress.XtraGrid.GridControl.AutoFilterRowHandle AndAlso e.Clicks = 2 Then
                    Try
                        With CType(Me.DetailGrid.FindForm, CRUDBase)
